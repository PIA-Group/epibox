...
for startup_file in [f for f in os.listdir(directory) if 'run' in f]:

    startup_time = '_'.join(startup_file.split('_')[3:])
    ...
    df = pd.read_csv(
        file_path, sep=', ', header=None, names=['timestamp', 'id', 'key']
    )
    ...
    for id in ids:
        a = df.loc[df['id'] == id]
        try:
            result = np.abs(a.iloc[1]['timestamp'] - a.iloc[0]['timestamp'])
            print(result)
    ...
    labels_timestamps += [[
        data_group.iloc[0]['timestamp'],
        data_group.iloc[-1]['timestamp']
    ]]

full_df.to_csv(os.path.join(directory, 'df.csv'))
